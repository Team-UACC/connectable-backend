...
class EventControllerTest {
    ...
    private static final EventResponse EVENT_RESPONSE_2 =
            new EventResponse(
                    EVENT_ID_2,
                    "test2",
                    "/connectable-events/image_0xtest.jpeg",
    ...
    private static final TicketResponse TICKET_RESPONSE_1 =
            new TicketResponse(
                    TICKET_ID_1,
                    10000,
                    "빅나티",
    ...
    private static final TicketResponse TICKET_RESPONSE_2 =
            new TicketResponse(
                    TICKET_ID_2,
                    10000,
                    "빅나티",
    ...
    void getEventsList() throws Exception {
        ...
        // expected
        mockMvc.perform(get("/events").contentType(APPLICATION_JSON))
    ...
    void getTicketList() throws Exception {
        // given
        given(eventService.getTicketList(EVENT_ID_1))
                .willReturn(List.of(TICKET_RESPONSE_1, TICKET_RESPONSE_2));

        // expected
        mockMvc.perform(get("/events/{event-id}/tickets", EVENT_ID_1).contentType(APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    void getTicketInfo() throws Exception {
        ...
        // expected
        mockMvc.perform(
                        get("/events/{event-id}/tickets/{ticket-id}", EVENT_ID_1, TICKET_ID_1)
